...
type ModalRbTematikProps = {
  ...
  onSave: (data?: RealisasiRbTematikResponse) => void
  idRbTematik: string
  idIndikatorRbTematik: string
  idTargetRbTematik: string
  kodeOpd: string
  nip: string
  initialBuktiPendukung?: string
}

type RealisasiRbTematikResponse = {
  id: number
  kode_opd: string
  nip: string
  tahun: string
...
export function ModalRbTematik({
  open,
  onOpenChange,
  kegiatanUtama,
  indikator,
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch(
        "/api/realisasi/laporanrbtematik/upload/file",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleSave = async () => {
    ...
    if (!idRbTematik || !idIndikatorRbTematik || !idTargetRbTematik) {
    ...
    console.log("payload realisasi tematik:", payload)

    setIsSubmitting(true)

    try {
      const res = await fetch("/api/realisasi/laporanrbtematik", {
        method: "POST",
        headers: {
      ...
      const data: RealisasiRbTematikResponse = await res.json()
      console.log("response realisasi tematik:", data)
