...
// response dari endpoint realisasi
type ApiRealisasi = {
  id: number
  kode_opd: string
  nip: string
...
// gabungkan data master dengan realisasi yang sudah tersimpan
function mergeRealisasi(
  rows: RealisasiRb[],
  realisasiList: ApiRealisasi[]
): RealisasiRb[] {
  // buat map berdasarkan id_indikator_rb_general
  // kalau ada beberapa bulan, ambil yang id paling besar (paling baru)
  const map = new Map<string, ApiRealisasi>()
  realisasiList.forEach((r) => {
    const key = r.id_indikator_rb_general
...
export function RbTable() {
  ...
  // State edit realisasi tahun berjalan
  ...
  // fetch data master RB + realisasi, lalu merge
  useEffect(() => {
    let cancelled = false
    ...
    async function load() {
      ...
      try {
        ...
        // 2. fetch realisasi (kalau userInfo sudah tersedia)
        if (userInfo?.nip && userInfo?.kodeOpd) {
          try {
            const realisasiRes = await fetch(
              `/api/realisasi/laporanrbgeneral/nip/${userInfo.nip}/kodeOpd/${userInfo.kodeOpd}/tahun/${tahun}/perencanaan`,
              { cache: "no-store" }
            )
            if (realisasiRes.ok) {
              const realisasiJson = await realisasiRes.json()
              // asumsi response: array, atau { data: array }
            ...
            // diamkan saja — data realisasi opsional
          }
        }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <ModalRbGeneral
        ...
        idRbGeneral={editingRow ? String(editingRow.idRb) : ""}
        idIndikatorRbGeneral={editingRow?.id ?? ""}
        idTargetRbGeneral={editingRow?.idTarget ?? ""}
